...
subtitle: 학습 기반 주행과 Rule-based 기준 경로를 함께 운용하도록 처음부터 설계한 비교 방식
...
목표는 학습 기반 주행의 비교적 부드러운 행동과 명시적인 규칙에 따라 동작하는 기존 경로 계획 방식의 특성을 한 시스템 안에서 비교하고 결합해보는 것이었다.
...
<figure class="project-media project-media--concept">
  ...
  <figcaption><strong>Hybrid architecture.</strong> Imitation Learning 출력과 Rule-based 기준 경로의 차이를 비교해 Imitation Learning 제어 또는 Rule-based 대체 제어를 선택한다.</figcaption>
</figure>

## Switching Logic

Imitation Learning이 기본적으로 차량을 주행하는 동안 Rule-based planner도 동시에 기준 경로를 계속 생성했다. Imitation Learning 출력과 Rule-based 기준 경로의 차이가 설정 기준보다 커지면 Rule-based 제어로 전환하고, 차이가 다시 줄어들면 Imitation Learning으로 복귀하도록 구성했다.

당시 사용한 정확한 전환 기준값은 보존된 자료에서 확인하지 못해 이 페이지에 숫자로 적지 않았다.

Rule-based planner는 기준 경로와 대체 제어로 사용했으며, 안전을 수학적으로 보장하는 별도의 safety layer는 아니었다.
...
<figure class="project-media">
  ...
  <figcaption><strong>Representative Hybrid driving in Town04.</strong> 개발 환경에서 주변 차량과 상호작용하며 주행하는 실제 장면이다. Hybrid의 전환 구조는 위 구성도에서 설명한다.</figcaption>
</figure>
